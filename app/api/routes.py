"""FastAPI route handlers for /api/ask, /api/ingest, /api/health."""
...
from app.api.schemas import AskRequest, AskResponse, Citation, IngestRequest, IngestResponse
...
def ask(request: AskRequest) -> AskResponse:
    ...
    # Fallback: if the model returned no [cite: ...] markers, build citations
    # from every chunk that was included in the prompt.
    if not citations:
...
def ingest(request: IngestRequest = IngestRequest()) -> IngestResponse:
    ...
    # Group pages by source file so we can process one document at a time
